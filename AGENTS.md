...
- **Transaction usage**: Flag any new or modified file under `web-api/src/business/` that performs a database write outside the async callback passed to `withTransaction` (imported from `@web-api/persistence/postgres/utils/transactions`). A call is a database write if it ultimately invokes `getDbWriter` (from `@web-api/persistence/postgres/database`); the three most common helpers that do this are `pgInsertInto`, `pgUpdateTable`, and `pgDeleteFrom` (all under `@web-api/persistence/postgres/utils/operation/`), but `getDbWriter` may also be called directly. Read-only operations that only call `getDbConnection` / `getDbReader` do not require a transaction boundary. Flag both the complete absence of a `withTransaction` call and writes that escape its callback scope (e.g., invoked after `await withTransaction(...)` resolves, or in a branch that bypasses it).
