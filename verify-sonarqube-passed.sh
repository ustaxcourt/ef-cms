...
OUTPUT=$(sonar-scanner -X -Dsonar.projectKey="${SONAR_KEY}" -Dsonar.branch.name="${branch_name}" -Dsonar.organization="${SONAR_ORG}" -Dsonar.projectBaseDir="${PROJECT_PATH}" -Dproject.settings="${PROJECT_PATH}/sonar-project.properties" -Dsonar.login="${SONAR_TOKEN}" -Dsonar.host.url="${FULL_URL}")
echo "${OUTPUT}"
