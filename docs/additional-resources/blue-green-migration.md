...
!> **Note:** This document contains outdated information about DynamoDB migrations. The project has migrated to Postgres. This document is kept for historical reference only.
...
2. Check to see if the **destination** exists and delete it if necessary. The specific steps depend on your current database setup.

#### Via the AWS Console

1. Identify the source and destination. Both should be the same value, and that is what the application is currently using as it its main data store. If they are both `alpha`, then the destination table is `beta`. If they are both `beta`, the destination table is `alpha`. If they are different, inspect the current color Lambda's environment config to determine what the application is currently using as its data store.
1. Delete the destination in `east`;
...
1. Remove a migration script item from the environment's deploy configuration that is expected to migrate, from the `migrationsToRun.ts` file. For example, if `migrationsToRun.ts` includes a script called `0001-test-script.ts`, delete that item from the deploy configuration.

## Explicitly Configure Deploy

1. Go into the environment's deploy configuration.
2. Change the `migrate` flag to `true`.
3. Change the value for the destination version to the destination you identified in the preparation steps (`alpha` or `beta`).
4. Delete the destination in `east`;
