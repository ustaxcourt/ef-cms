...
|`npm run reset-cases ENV ELASTICSEARCH_ENDPOINT`|Removes all cases from the database and ElasticSearch for an environment in AWS.|
|`npm run reindex:elasticsearch -- ENV ELASTICSEARCH_ENDPOINT`|Deletes and recreates ElasticSearch indices and reindexes all database records for an environment in AWS.|
