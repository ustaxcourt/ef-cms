...
5. Edit the deploy-app.sh to comment out ONLY the terraform plan and apply commands (not init)
    # terraform plan -out execution-plan
    # terraform apply -auto-approve execution-plan
6. Set up terraform for the specific environment in question by running the deploy-app.sh script (this will eventually run the `terraform init` command)  (example: `../bin/deploy-app.sh "$ENV"`)
