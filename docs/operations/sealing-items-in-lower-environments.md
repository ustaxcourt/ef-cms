...
The terraform config also permits a specified lambda in a lower environment to subscribe to it. We attach the `sns_topic_policy` that grants access to those lambda functions to principals identified by the environment variable `PRODLIKE_LOWER_ENV_ACCOUNT_IDS`.

## In the Lower Environments

In any environment that is not production, a new lambda named `seal_in_lower` is created to subscribe to the SNS topic above. These lambdas are only created if the `AWS_ACCOUNT_ID` does NOT match the `PROD_ENV_ACCOUNT_ID`.
...
aws sns subscribe \
    ...
    --protocol "lambda" \
    --notification-endpoint "arn:aws:lambda:us-east-1:${AWS_ACCOUNT_ID}:function:seal_in_lower_dev_blue"
aws sns subscribe \
    --topic-arn "arn:aws:sns:us-east-1:${PROD_ENV_ACCOUNT_ID}:seal_notifier" \
    --protocol "lambda" \
    --notification-endpoint "arn:aws:lambda:us-east-1:${AWS_ACCOUNT_ID}:function:seal_in_lower_dev_blue"
