...
- Create the lambda roles & policies needed for the lambdas that run the backend:
     - `cd iam/terraform/main && ../bin/deploy-app dev`
     - `cd iam/terraform/main && ../bin/deploy-app stg`
     - `cd iam/terraform/main && ../bin/deploy-app prod`
...
7. Add the following:
     ...
     - `CLOUDWATCH_ROLE_ARN_DEV` (the arn output after running terraform in the iam/terraform/main dir)
     - `CLOUDWATCH_ROLE_ARN_STG` (the arn output after running terraform in the iam/terraform/main dir)
     - `CLOUDWATCH_ROLE_ARN_PROD` (the arn output after running terraform in the iam/terraform/main dir)
     - `POST_CONFIRMATION_ROLE_ARN_DEV` (the arn output after running terraform in the iam/terraform/main dir)
     - `POST_CONFIRMATION_ROLE_ARN_STG` (the arn output after running terraform in the iam/terraform/main dir)
     - `POST_CONFIRMATION_ROLE_ARN_PROD` (the arn output after running terraform in the iam/terraform/main dir)
