...
> - If terraform needs to be updated, deploy to an exp environment first using your local Terraform.
...
1. `npm audit`: Informs us of known security vulnerabilities. If transitive dependencies are vulnerable, use the resolutions block in `package.json` to specify version overrides.
   - If a dependency is vulnerable and has no fix, consider replacing it with an alternative.

   > NOTE: If any npm packages are updated but the `package-lock.json` file is not updated, increment the node cache version in the circle config. You can do this by searching within `config.yml` for vX-npm and vX-cypress where X is the current version of the cache key, then increment the version found.
