...
resource "aws_iam_user_policy_attachment" "ci_cd_policy_attachment" {
...
resource "aws_iam_user_policy_attachment" "ci_cd_route53_policy_attachment" {
  user       = aws_iam_user.ci_cd.name
  policy_arn = aws_iam_policy.ci_cd_route53_policy.arn
}
...
resource "aws_iam_user_policy_attachment" "ci_cd_iam_policy_attachment" {
  user       = aws_iam_user.ci_cd.name
  ...
  "Statement": [
    {
      ...
      "Resource": [
        ...
        "arn:aws:iam::${data.aws_caller_identity.current.account_id}:role/*-rum-unauthenticated-role",
