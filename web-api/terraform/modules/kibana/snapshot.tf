...
provider "opensearch" {
  url = var.es_info_cluster_create ? "https://${aws_opensearch_domain.efcms-logs[0].endpoint}" : "https://${jsondecode(data.aws_secretsmanager_secret_version.info_cluster[0].secret_string)["endpoint"]}"
}
