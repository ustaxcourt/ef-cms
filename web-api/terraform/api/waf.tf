resource "aws_wafv2_web_acl" "apis" {
  ...
  rule {
    ...
    visibility_config {
      ...
      metric_name                = "block_banned_ip_addresses_${var.environment}_${var.current_color}"
...
resource "aws_wafv2_ip_set" "banned_ipv4_ips" {
  name               = "banned_ipv4_ips_${var.environment}_${var.current_color}"
  scope              = "REGIONAL"
...
resource "aws_wafv2_ip_set" "banned_ipv6_ips" {
  name               = "banned_ipv6_ips_${var.environment}_${var.current_color}"
  scope              = "REGIONAL"
