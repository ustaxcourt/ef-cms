...
resource "aws_cognito_user_pool" "data_viewers" {
  name = "efcms-${var.environment}-data-viewers"
  password_policy {
    minimum_length                   = 8
    require_lowercase                = true
    require_uppercase                = true
...
resource "aws_cognito_user_pool_domain" "data_viewers" {
  domain       = "ef-cms-data-viewers-${var.cognito_suffix}"
  user_pool_id = aws_cognito_user_pool.data_viewers.id
}

resource "aws_cognito_identity_pool" "data_viewers" {
...
resource "aws_iam_role" "es_kibana_role" {
  name               = "es_kibana_role_${var.environment}"
