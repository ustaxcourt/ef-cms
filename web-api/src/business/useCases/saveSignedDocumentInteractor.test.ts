...
describe('saveSignedDocumentInteractor', () => {
  ...
  const getCaseEntityFromLastPersistCall = () => {
    const args = updateCaseAndAssociations.mock.calls.at(-1)?.[0];
    if (!args) {
      throw new Error('expected updateCaseAndAssociations to have been called');
    }
    return args.caseToUpdate;
  };
  ...
  it('should add the signed Stipulated Decision to the case given a Proposed Stipulated Decision', async () => {
    ...
    const caseEntity = getCaseEntityFromLastPersistCall();
