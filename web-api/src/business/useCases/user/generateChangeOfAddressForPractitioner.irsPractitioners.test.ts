...
describe('generateChangeOfAddress', () => {
  ...
  beforeEach(() => {
    ...
    applicationContext
      .getPersistenceGateway()
      .getUserById.mockReturnValue(mockIrsPractitioner);
  });
  ...
  describe('BUGS', () => {
    it('should use the latest user info in persistence when updating contactInfo', async () => {
      const TEST_PENDING_EMAIL = 'TEST_PENDING_EMAIL@example.com';
      ...
      const updatedUser = updateUserCalls[0][0].user;
