...
import { CaseDeadline, RawCaseDeadline } from '@shared/business/entities/CaseDeadline';
    ...
    caseEntities = await settlePromises(
      caseEntities.map(async caseEntity => {
        ...
        if (isSubjectCase && docketEntryEntity.shouldAutoGenerateDeadline()) {
          ...
          const deadline = (await applicationContext.getUseCaseHelpers().autoGenerateDeadline({
            deadlineDate: docketEntryEntity.date,
            description: docketEntryEntity.getAutoGeneratedDeadlineDescription(),
            subjectCaseEntity,
          }))[0];
          
          if(caseEntity.leadDocketNumber === caseEntity.docketNumber) {
            const consolidatedCases = await getConsolidatedCases({ leadDocketNumber: caseEntity.leadDocketNumber });
