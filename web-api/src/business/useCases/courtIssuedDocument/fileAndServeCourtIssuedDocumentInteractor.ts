  ...
  try {
    ...
    caseEntities = await settlePromises(
      caseEntities.map(async caseEntity => {
        ...
        if (isSubjectCase && docketEntryEntity.shouldAutoGenerateDeadline() || (isSubjectCase && form.dueDate && form.orderType && docketEntryEntity.eventCode === 'O')) {
          ...
          /* 
            getAutoGeneratedDeadlineDescription gets the description based on docketEntryEntity.eventCode, but 
            Status Report Orders has an eventCode of 'O' which is very generic.
          */
          const description = (form.dueDate && form.orderType) 
            ? orderDescriptions[form.orderType]
