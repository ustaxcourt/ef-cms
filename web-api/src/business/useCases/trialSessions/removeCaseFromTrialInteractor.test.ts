...
describe('removeCaseFromTrialInteractor', () => {
  ...
  it('sets the associatedJudge and caseStatus when provided', async () => {
    ...
    const updatedCase =
      updateCaseAndAssociations.mock.calls[0][0].caseToUpdate;
    expect(updatedCase.associatedJudge).toEqual('Judge Dredd');
    expect(updatedCase.associatedJudgeId).toEqual(
      'e5eaf0ac-6a1f-4a5d-a44d-d59f199b7ab5',
    );
    expect(updatedCase.status).toEqual(CASE_STATUS_TYPES.cav);
  });
