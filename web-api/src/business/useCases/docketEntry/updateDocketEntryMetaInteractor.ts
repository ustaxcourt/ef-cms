...
export const updateDocketEntryMeta = async (
  ...
  const certificateOfServiceUpdated =
    ...
    const isSystemGenerated =
      originalDocketEntry.filedByRole === 'System' ||
      originalDocketEntry.isAutoGenerated === true;

    let shouldGenerateCoversheet = false;
    if (!isSystemGenerated) {
      shouldGenerateCoversheet = shouldGenerateCoversheetForDocketEntry({
