...
mkdir -p coverage
STAGE="${slsStage}" DYNAMODB_ENDPOINT="dynamodb.${region}.amazonaws.com" API_REGION=${region} API_STAGE=${slsStage} API_TARGET=${restApiId} DEBUG_FD=1 DEBUG=http:response ./node_modules/.bin/artillery run -v "{\"token\": \"$token\"}" ./smokeTest.yml --output ./coverage/artillery_smoke_test.json
