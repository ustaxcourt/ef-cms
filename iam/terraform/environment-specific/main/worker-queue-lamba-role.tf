resource "aws_iam_role" "iam_worker_queue_lambda_role" {
  name = "iam_worker_queue_lambda_role_${var.environment}"
...
// TODO 10007: Can we just use the api_role instead of creating a new role for this queue.
resource "aws_iam_role_policy" "iam_worker_queue_lambda_policy" {
  name = "iam_worker_queue_lambda_policy_${var.environment}"
  role = aws_iam_role.iam_worker_queue_lambda_role.id
