const AWS = require('aws-sdk');

const check = (value, message) => {
  if (!value) {
    console.log(message);
    process.exit(1);
  }
};

const { DEPLOYING_COLOR, EFCMS_DOMAIN, ENV } = process.env;

check(DEPLOYING_COLOR, 'You must have DEPLOYING_COLOR set in your environment');
check(EFCMS_DOMAIN, 'You must have EFCMS_DOMAIN set in your environment');
check(ENV, 'You must have ENV set in your environment');

// const route53 = new AWS.Route53();

const REGIONS = ['us-west-1'];

const run = async () => {
  for (const region of REGIONS) {
    const apigateway = new AWS.APIGateway({
      region,
    });

    const { items } = await apigateway.getRestApis({}).promise();

    const apiGatewayRecord = items.find(
      record => record.name === `gateway_api_${ENV}_${DEPLOYING_COLOR}`,
    );

    apigateway.createBasePathMapping(
      {
        domainName: `api.${EFCMS_DOMAIN}`,
        restApiId: apiGatewayRecord.id,
        stage: ENV,
      },
      (err, data) => {
        if (err) console.log(err, err.stack);
        // an error occurred
        else console.log(data); // successful response
      },
    );
  }

  // const apiGatewayRecord = items.find(
  //   customDomain =>
  //     customDomain.domainName === `api-${DEPLOYING_COLOR}.${EFCMS_DOMAIN}`,
  // );

  // await route53
  //   .changeResourceRecordSets({
  //     ChangeBatch: {
  //       Changes: [
  //         {
  //           Action: 'UPSERT',
  //           ResourceRecordSet: {
  //             AliasTarget: {
  //               DNSName: `api-${DEPLOYING_COLOR}.${EFCMS_DOMAIN}`,
  //               EvaluateTargetHealth: false,
  //               HostedZoneId: 'Z3695GT08DFEQ',
  //             },
  //             Name: `api.${EFCMS_DOMAIN}`,
  //             Type: 'A',
  //           },
  //         },
  //       ],
  //       Comment: `The API for api.${EFCMS_DOMAIN}`,
  //     },
  //     HostedZoneId: 'Z3695GT08DFEQ',
  //   })
  //   .promise();
};

run();
