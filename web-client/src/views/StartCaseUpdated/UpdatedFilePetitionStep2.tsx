...
export const useValidationFocus = validationErrors => {
  ...
  useEffect(() => {
    if (!hasFocused.current && validationErrors) {
      ...
      const collectElements = (errors, prefix = '') => {
        for (const key in errors) {
          if (typeof errors[key] === 'object' && errors[key] !== null) {
            collectElements(errors[key], `${prefix}${key}.`);
      ...
      collectElements(validationErrors);
        ...
        {['Yes', 'No'].map((isSpouseDeceased, idx) => (
          <div className="usa-radio usa-radio__inline" key={isSpouseDeceased}>
            <input
              ...
              ref={registerRef !== undefined && registerRef('isSpouseDeceased')}
              type="radio"
    ...
    <>
      ...
      >
        <input
          ...
          ref={registerRef !== undefined && registerRef('hasSpouseConsent')}
          type="checkbox"
          onChange={e => {
            updateFormValueSequence({
              key: e.target.name,
