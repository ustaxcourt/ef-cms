...
import { getDocumentQCInProgressForUserAction } from './getDocumentQCInProgressForUserAction';
...
describe('getDocumentQCInboxForUserAction', () => {
  ...
  it('should make a call to get the current user', async () => {
    await runAction(getDocumentQCInProgressForUserAction, {
      modules: {
        presenter,
      },
    });
  ...
  it('should return the retrieved work items as props', async () => {
    const { output } = await runAction(getDocumentQCInProgressForUserAction, {
      modules: {
        presenter,
      },
    });
