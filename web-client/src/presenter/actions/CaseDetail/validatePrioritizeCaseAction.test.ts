...
import { validatePrioritizeCaseAction } from './validatePrioritizeCaseAction';

describe('validatePrioritizeCaseAction', () => {
  let successStub;
  let errorStub;
  ...
  it('should call path.success and not path.error if a reason is set on state.modal', async () => {
    await runAction(validatePrioritizeCaseAction, {
      modules: {
        presenter,
      },
      state: {
