...
TF_VAR_my_s3_state_bucket="${BUCKET}" TF_VAR_my_s3_state_key="${KEY}" terraform apply -auto-approve -var "dns_domain=${EFCMS_DOMAIN}" -var "environment=${ENVIRONMENT}" -var "git_access_token=${GIT_ACCESS_TOKEN}" -var "product_keys=${PRODUCT_KEYS}" -var "dynamsoft_zip_name=${DYNAMSOFT_ZIP_NAME}" -var "dynamsoft_repo=${DYNAMSOFT_REPO}"