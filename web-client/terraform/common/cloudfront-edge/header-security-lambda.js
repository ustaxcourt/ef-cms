...
exports.handler = (awsEvent, handlerContext, callback) => {
  ...
  const contentSecurityPolicy = [
    ...
    `connect-src blob: ${subdomainsUrl} ${applicationUrl} ${cognitoUrl} ${s3Url} ${dynamsoftUrlEnv} ${dynamsoftUrlProd} ${dynamsoftUrlStaging} ${localUrl} ${websocketUrl} ${localWebsocketUrl} ${pdfjsExpressUrl}`,
    "default-src 'none'",
    "manifest-src 'self'",
    `form-action ${applicationUrl} ${subdomainsUrl}`,
    `object-src ${subdomainsUrl} ${applicationUrl} ${s3Url}`,
    `script-src 'self' 'unsafe-inline' ${dynamsoftUrlEnv} ${dynamsoftUrlProd} ${dynamsoftUrlStaging} ${statuspageUrl} resource://pdf.js`,
    `worker-src blob: ${subdomainsUrl}`,
    `style-src 'self' 'unsafe-inline' ${dynamsoftUrlEnv} ${dynamsoftUrlProd} ${dynamsoftUrlStaging}`,
