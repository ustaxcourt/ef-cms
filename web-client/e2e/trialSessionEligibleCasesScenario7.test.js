...
describe('Trial Session Eligible Cases - Scenario 7 - Case status is automatically set to “Calendared for Trial" once a case is scheduled to a session', () => {
  beforeEach(() => {
  ...
  const trialLocation = `Starkville, Mississippi, ${Date.now()}`;
  const overrides = {
    maxCases: 1,
  ...
  describe(`Result: Case #1 and #2 should show as eligible for '${trialLocation}' session`, () => {
    ...
    it(`Case #1 and #2 should show as eligible for '${trialLocation}' session`, async () => {
      ...
      expect(test.getState('eligibleCases').length).toEqual(2);
  ...
  describe(`Result: Case #1 is assigned to '${trialLocation}' session and case status is updated to “Calendared for Trial”`, () => {
    petitionsClerkLogIn(test);

    it(`Case #1 is assigned to '${trialLocation}' session and case status is updated to “Calendared for Trial”`, async () => {
      await test.runSequence('gotoTrialSessionDetailSequence', {
        trialSessionId: test.trialSessionId,
      });
