...
describe('compare the pdf documents against the expected images', () => {
  const pdfs = [
  ...
  pdfs.forEach(testEntry => {
    it(`testing ${testEntry.fileName}`, async () => {
      const storeOutputImage = fromPath(
        path.join(outputPath, testEntry.fileName),
        {
          density: 100,
          format: 'png',
          height: 1000,
          saveFilename: testEntry.fileName,
          savePath: './shared/test-output',
          width: 800,
        },
      );
      await storeOutputImage(testEntry.pageNumber);

      const actualImage = PNG.sync.read(
        fs.readFileSync(
          `./shared/test-output/${testEntry.fileName}.${testEntry.pageNumber}.png`,
        ),
      );
      const expectedImage = PNG.sync.read(
        fs.readFileSync(
          `./shared/test-pdf-expected-images/${testEntry.fileName}.${testEntry.pageNumber}.png`,
        ),
      );
      ...
      fs.writeFileSync(
        `./shared/test-output/diff/${testEntry.fileName}.${testEntry.pageNumber}.png`,
        PNG.sync.write(diff),
      );
