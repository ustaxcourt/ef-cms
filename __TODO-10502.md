...
- Get streaming to work from PostGres to OpenSearch (independent research, pair)
  - This has been a bear. TL;DR: resilient, scalable, real-time change data capture is difficult for serverless Aurora. We are trying therefore to move queries from Open Search to Postgres.
- Make sure getCasesMetadataByUserId (previously called getCasesForUser) is working. It relies on processPractitionerMappingEntries streaming things into PractitionerOnCaseTable
- Add migrations for the stubbed out database-types
- There are a bunch of queries in postgres/reports/caseSearch. The ones that are still Open Search need to be moved to Postgres, and the ones that have been moved need to be tested and optimized.
