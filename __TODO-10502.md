...
- It looks like consecutive capital letters are translated into JS objects incorrectly (e.g., hasConsentedToEservice on Petitioner --> hasConsentedToEservice). How to best address?

- We are making redundant calls to getDbWriter/reader (for instance, in updateCase); each time will lead to a connection health check. Can we avoid this?
...
- get rid of case-mapping (pair):
  ...
  - advancedDocumentSearch
    - Case title, petitioner advanced syntax: get this working
...
- Write script to delete Case OpenSearch data (efcms-case, case-mapping) (solo)
- Write script to delete Case Dynamo data (solo)
- Upsert cases doesn't take case statistics and related tables into account
- Make sure updating petitioners still work and applies changes to both postgres and opensearch 
- Make sure to use Case.getDocketNumberWithSuffix instead of arbitrary ternaries
