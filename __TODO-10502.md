...
- test script to delete Case Dynamo data (solo)
  - Let's do this before our next migration
...
- Check our SQS Queue Type and Limits
  - Because we reload the case in our lambda handler, we don't need a FIFO queue
  - Because we don't need a FIFO queue, we have unlimited throughput
- Make sure 10502-dxox has changes that are in test:
  - I think this is done, but good to double-check
  - Test PR #5933 DONE
  - Test PR #5928 DONE
  - Test PR #5924 DONE
  - Test PR #5911 NOT DONE, need to discuss typing and database-types

- Query optimization:
  - getCaseByDocketNumber
    - 1) I think we actually only need docketEntryId for workItem, which might shave a good deal of time off of our query. I suggest we store docketEntryId as a new indexed column (right now, it is a key/value in the docketEntry json column) and query for that.
    - 2) For consolidated cases, do we need getCasesMetadataWithCounselByLeadDocketNumber? Or can we just get the case metadata? That could be another option for faster queries here.
    - 3) I experimented with clustering work items by docket number, but that did not seem to have a noticeable effect. The bigger effect seems to be the amount of data we are selecting. Select * from dw_work_item is slow compared to Select dw_work_item_id. Hence my suggestion in 1.
    - 4) Custom case report sometimes times out
