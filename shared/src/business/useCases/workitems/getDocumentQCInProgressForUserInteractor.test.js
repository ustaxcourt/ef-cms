...
} = require('./getDocumentQCInProgressForUserInteractor');

describe('getDocumentQCInProgressForUserInteractor', () => {
  let mockWorkItem = {
    createdAt: '',
    docketEntry: {
      sentBy: 'petitioner',
  ...
  it('throws an error if the user does not have access to the work item', async () => {
    ...
    let error;
    try {
      await getDocumentQCInProgressForUserInteractor(applicationContext, {
        userId: '123',
      });
    } catch (e) {
      error = e;
  ...
  it('queries workItems for the given userId', async () => {
    ...
    expect(
      applicationContext.getPersistenceGateway().getDocumentQCInProgressForUser
  ...
  it('filters the returned workItems for the given user', async () => {
    ...
    applicationContext
      .getPersistenceGateway()
      .getDocumentQCInProgressForUser.mockReturnValue([workItem]);
