...
} = require('./getDocumentQCInProgressForSectionInteractor');

describe('getDocumentQCInProgressForSectionInteractor', () => {
  let mockWorkItem = {
    createdAt: '',
    docketEntry: {
      sentBy: 'petitioner',
  ...
  it('throws an error if the user does not have access to the work item', async () => {
    ...
    let error;
    try {
      await getDocumentQCInProgressForSectionInteractor(applicationContext, {
        section: DOCKET_SECTION,
      });
    } catch (e) {
      error = e;
  ...
  it('queries workItems for the DOCKET_SECTION when the DOCKET_SECTION is passed', async () => {
    ...
    expect(
      applicationContext.getPersistenceGateway()
        .getDocumentQCInProgressForSection.mock.calls[0][0].section,
  ...
  it('queries workItems for the DOCKET_SECTION with a judgeUserName', async () => {
    ...
    expect(
      applicationContext.getPersistenceGateway()
        .getDocumentQCInProgressForSection.mock.calls[0][0].judgeUserName,
    ).toEqual('Ashford');
  });
});
