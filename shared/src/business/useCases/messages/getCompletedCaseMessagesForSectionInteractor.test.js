...
} = require('./getCompletedCaseMessagesForSectionInteractor');
const {
  UnauthorizedError,
...
describe('getCompletedCaseMessagesForSectionInteractor', () => {
  ...
  it('retrieves the case messages from persistence and returns them', async () => {
    ...
    applicationContext
      ...
      .getCompletedSectionInboxMessages.mockReturnValue([caseMessageData]);

    const returnedMessages = await getCompletedCaseMessagesForSectionInteractor(
      {
        applicationContext,
