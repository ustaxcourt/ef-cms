...
} = require('./getOutboxCaseMessagesForUserInteractor');
const {
  UnauthorizedError,
...
describe('getOutboxCaseMessagesForUserInteractor', () => {
  ...
  it('retrieves the case messages from persistence and returns them', async () => {
    ...
    applicationContext
      ...
      .getUserOutboxMessages.mockReturnValue([caseMessageData]);

    const returnedMessages = await getOutboxCaseMessagesForUserInteractor({
      applicationContext,
      userId: caseMessageData.userId,
    });
