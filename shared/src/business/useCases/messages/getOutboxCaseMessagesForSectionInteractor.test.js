...
} = require('./getOutboxCaseMessagesForSectionInteractor');
const {
  UnauthorizedError,
...
describe('getOutboxCaseMessagesForSectionInteractor', () => {
  ...
  it('retrieves the case messages from persistence and returns them', async () => {
    ...
    applicationContext
      ...
      .getSectionOutboxMessages.mockReturnValue([caseMessageData]);

    const returnedMessages = await getOutboxCaseMessagesForSectionInteractor({
      applicationContext,
