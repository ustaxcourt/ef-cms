...
export const fileAndServeCourtIssuedDocumentInteractor = async (
  ...
  const shouldAutoGenerateDeadlineForDocketEntry =
    shouldAutoGenerateDeadline(docketEntryToServe);

  if (shouldAutoGenerateDeadlineForDocketEntry) {
    await autoGenerateDeadline({
      applicationContext,
      deadlineDate: form.date,
      description: autoGeneratedDeadlineDescription(docketEntryToServe),
      subjectCaseEntity,
    });
  }
...
const autoGenerateDeadline = async ({
  applicationContext,
  deadlineDate,
  description,
  subjectCaseEntity,
