...
export class GenerateSuggestedTermModal extends JoiValidationEntity {
  ...
  constructor(
    rawProps: any,
    entityName: string = 'GenerateSuggestedTermModal',
  ) {
    super(entityName);
  ...
  static VALIDATION_RULES = {
    ...
    termStartDate: JoiValidationConstants.DATE_RANGE_PICKER_DATE.greater('now')
      .required()
      .messages({
        '*': 'Enter date in format MM/DD/YYYY.',
        'date.greater': 'Start date cannot be in the past. Enter a valid date.',
      }),
  };

  getValidationRules() {
    return GenerateSuggestedTermModal.VALIDATION_RULES;
  }
}

export type RawGenerateSuggestedTermModal = ExcludeMethods<
  Omit<GenerateSuggestedTermModal, 'entityName'>
>;
