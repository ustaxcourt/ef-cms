...
describe('generateFiledBy', () => {
  ...
  it('should set filedBy when the docket entry form is NOTW', () => {
    const docketEntry = new DocketEntry(
      {
        ...mockDocketEntry,
        ...
        filers: [mockPrimaryContactId],
        isAutoGenerated: false,
        filedBy: 'thisisfiledby',
      },
      { authorizedUser: mockIrsPractitionerUser, petitioners: mockPetitioners },
    );
    expect(docketEntry.filedBy).toEqual('thisisfiledby');
  });
});
