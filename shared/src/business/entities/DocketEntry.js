...
DocketEntry.prototype.generateFiledBy = function (petitioners) {
  ...
  if (this.eventCode === 'SOC') {
    console.log('shouldGenerateFiledBy', shouldGenerateFiledBy);
  }
  if (shouldGenerateFiledBy) {
    ...
    if (
      ...
      // can we assume that if docketEntry has a practitioner, that's who filed it?
      // from RequestAccessReview, if privatePractitioner, setPractitionerOnFormAction
      // and within uploadExternalDocumentsAction eventually call fileExternalDocumentInteractor
      // which has baseMetadata.practitioner
      ...
      if (filedByPrivatePractitioner) {
        if (this.eventCode === 'SOC') {
          console.log('filedByPrivatePractitioner', filedByPrivatePractitioner);
        }
        this.filedBy = filedByPrivatePractitioner.name;
        if (this.eventCode === 'SOC') {
          console.log(
            'set filedBy to privatePractitioner',
            filedByPrivatePractitioner.name,
            this.eventCode,
          );
        }
      }
