 ...
 * FIXME
 */
const autogenerateDeadlineDescription = rawDocketEntry => {
  const thing = {
    OAP: 'Amended Petition Due',
    OF: 'Filing Fee Due',
  };
  if (Object.keys(thing).includes(rawDocketEntry.eventCode)) {
    return thing[rawDocketEntry.eventCode];
  }
};
 ...
 */
const shouldAutoGenerateDeadline = rawDocketEntry => {
  // find the one whose eventCode matches the docket entry's event code
  // if the docket entry's event code is in this list then return true
  return Object.entries(AUTO_GENERATED_DEADLINE_DOCUMENT_TYPES).some(
    item => item.eventCode === rawDocketEntry.eventCode,
  );
};
 ...
 * @returns {Boolean} true i
 */
DocketEntry.prototype.autoGeneratedDeadline = function () {
  return this.shouldAutoGenerateDeadline.deadlineDescription;
};
...
module.exports = {
  ...
  autogenerateDeadlineDescription,
