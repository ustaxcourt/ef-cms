 ...
 * @constructor
 */
function CustomCaseInventorySearch() {
  this.entityName = 'JudgeActivityReportSearch';
}

CustomCaseInventorySearch.prototype.init = function init(rawProps = {}) {
  this.startDate = rawProps.startDate;
  this.endDate = rawProps.endDate;
};

CustomCaseInventorySearch.VALIDATION_ERROR_MESSAGES = {
  endDate: [
    {
...
CustomCaseInventorySearch.schema = joi.object().keys({
...
joiValidationDecorator(
  ...
  CustomCaseInventorySearch.schema,
  CustomCaseInventorySearch.VALIDATION_ERROR_MESSAGES,
);

exports.CustomCaseInventorySearch = validEntityDecorator(
  CustomCaseInventorySearch,
);
