...
- Node v10.15.3
- yarn 1.17.3 (and/or npm v6.4.1)

### Setup

Both the front-end (`/web-client`) and API (`/web-api`) share code that exists in `/shared`. Before you can run either, you need to run `yarn install` inside the top-level directory.
...
- Run `cd web-client && yarn start:client:no-scanner` to start the UI without Dynamsoft (or if you don't have a scanner)
...
To use Cognito, start the web client with `yarn dev:cognito` (instead of `yarn start`) You can then log in with:
