...
import { getHealthCheckInteractor } from '@shared/proxies/health/getHealthCheckProxy';
...
export function HealthCheckPage() {
  const {
    ...
    queryKey: ['health'],
    queryFn: () => {
      return getHealthCheckInteractor(applicationContextPublic);
    },
  });
...
export function HealthCheck({ health }: { health: ApplicationHealth }) {
  const components = {
    cognito: (
      <RenderHealthStatus
        id="cognito"
        ...
        <Mobile>
          ...
          <div className="grid-row">
            <div className="card width-full">
              ...
              <div className="margin-left-205 margin-bottom-205 grid-row">
                <div className="health-check-text grid-col-9">efcmsDeploy</div>
                {components.dynamoEfcmsDeploy}
              </div>
            </div>
