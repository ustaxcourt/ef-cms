...
import {
  ...
  publicTrialSessionDetailsComputed,
  TrialSessionPublicCaseRow,
} from 'web-client-public/src/routes/trial-session-detail/PublicTrialSessionDetailsComputed';

export function PublicTrialSessionsDetailsPage() {
  ...
  const {
    ...
    queryKey: ['trialSession', trialSessionId],
    queryFn: () =>
      getPublicTrialSessionDetailsInteractor(applicationContextPublic, {
        trialSessionId,
      }),
  });
  ...
  if (error) return <div>Error loading trial session.</div>;
}
