...
describe('before filing a petition', () => {
  beforeEach(() => {
    (cy as any).login('petitioner', 'before-filing-a-petition');
  });
...
describe('creation form', () => {
  before(() => {
    (cy as any).login('petitioner', 'file-a-petition/step-1');
  });
  ...
  it('case list table reflects newly-added record', () => {
    ...
    (cy as any).goToRoute('/file-a-petition/step-1');
    (cy as any).goToRoute('/');
...
describe('can view case detail', () => {
  before(() => {
    ...
    (cy as any).login('petitioner', `/case-detail/${createdDocketNumber}`);
