import { loginAsDocketClerk, loginAsPetitionsClerk1 } from '../../../../helpers/authentication/login-as-helpers';
...
describe('Trial Session Eligible Cases Journey', () => {
    ...
    it('should create trial session and cases, then verify eligible cases are properly assigned', () => {
        ...
        cy.then(() => {
            ...
            cy.get('[data-testid="remove-from-trial-session-disposition-textarea"]').type('testing');
            cy.get('[data-testid="modal-button-confirm"]').click();
            cy.get('[data-testid="success-alert"]').should('exist');

            // Verify case is no longer calendared
            cy.get('[data-testid="case-status"]').should('not.contain', CASE_STATUS_TYPES.calendared);
