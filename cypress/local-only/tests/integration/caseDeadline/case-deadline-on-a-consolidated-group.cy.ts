...
describe('Case Deadline - Consolidated Group', () => {
  it('should add deadline to lead case and see edit abilities for deadline transferred when lead case is removed from group', () => {
    setupConsolidatedGroup().then(
      ({ memberDocketNumbers, leadDocketNumber }) => {
        loginAsDocketClerk1();
        ...
        cy.get('[data-testid="tab-tracked-items"]').click();
        cy.get('[data-testid="case-deadline-edit-button"]').should(
          'be.visible',
        );
...
const setupConsolidatedGroup = (): Cypress.Chainable<GroupInfoType> => {
  return createAndServeConsolidatedGroup({
