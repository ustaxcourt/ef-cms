...
/**
 * Private practitioner e-files a Motion (not a Motion for Leave) alongside
 * an "Exhibit in Support" (EXS) supporting document, on a case where the
 * Petition has already been served to the IRS. Mirrors the TestRail manual
 * case for story 10192.
 *
 * PDF-content assertions (paging through the preview modal, comparing the
 * printed receipt's contents against the Docket Record) aren't exercised
 * here — no spec in this suite asserts against rendered PDF content, so
 * this test instead confirms the receipt link is present and pins the
 * coversheet-driven page counts, which is the suite's existing convention
 * for verifying coversheets were applied.
 */
