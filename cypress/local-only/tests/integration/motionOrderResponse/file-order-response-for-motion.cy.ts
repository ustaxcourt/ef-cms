...
describe('file motion response order', () => {
  ...
  const firstPdfLineJustThisCase =
    'On June 28, 2024, petitioner filed status report was filed (Document no.';
  ...
  describe('judge', () => {
    before(() => {
      loginAsPetitionsClerk1();
      trialSessionId = createTrialSession({
        trialLocation: 'Birmingham, Alabama',
      });
      console.log(`trialSessionId: ${trialSessionId}`);
      cy.log(`trialSessionId: ${trialSessionId}`);
      docketNumber = createAndServePaperPetition({ yearReceived: '2025' });
      console.log(`caseDocketNumber: ${docketNumber}`);
      cy.log(`caseDocketNumber: ${docketNumber}`);
    });
    beforeEach(() => {
      loginAsColvin();
    });
    ...
    describe('filing a motion response order from document view', () => {
      it('should be able to select motion from docket record', () => {
        cy.visit(`/case-detail/${docketNumber}`);
        cy.get('#tab-document-view').click();
        // cy.get('[data-testid="motion-response-order-button"]').click();
        // cy.get('[data-testid="select-motion"]').should('exist');
      });
    });
