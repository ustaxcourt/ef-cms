...
exports.createTrialSession = (testData, overrides = {}) => {
  ...
  cy.get('#start-time-hours').type(faker.datatype.number({ max: 11, min: 6 }));
  ...
  cy.get(
    `#judgeId option:contains(${(overrides as any).offboardedJudge})`,
  ).should('not.exist');
...
exports.goToTrialSession = trialSessionId => {
  (cy as any).goToRoute(`/trial-session-detail/${trialSessionId}`);
...
exports.goToTrialSessionWorkingCopy = ({
  ...
  (cy as any).goToRoute(`/trial-session-working-copy/${trialSessionId}`);
