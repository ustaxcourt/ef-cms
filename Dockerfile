...
RUN curl "https://awscli.amazonaws.com/awscli-exe-linux-x86_64-2.9.22.zip" -o "awscliv2.zip" && \
  unzip awscliv2.zip && \
  ./aws/install && \
  rm -rf awscliv2.zip
...
RUN wget -q -O terraform.zip https://releases.hashicorp.com/terraform/1.3.8/terraform_1.3.8_linux_amd64.zip && \
