...
RUN curl "https://awscli.amazonaws.com/awscli-exe-linux-x86_64-2.11.2.zip" -o "awscliv2.zip" && \
  unzip awscliv2.zip && \
  ./aws/install && \
  rm -rf awscliv2.zip
...
RUN wget -q -O terraform.zip https://releases.hashicorp.com/terraform/1.4.0/terraform_1.4.0_linux_amd64.zip && \
