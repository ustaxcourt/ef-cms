...
let itemsScanned = 0;
const docketEntries: RawDocketEntry[][] = Array.from({
...
async function scanDynamoAndBatchUpsertDocketEntriesToPostgres(params: ScanCommandInput) {
  ...
  do {
    ...
    for (const record of items as TDynamoRecord[]) {
      if (record.sk.startsWith('docket-entry|')) {
        ...
        docketEntries[params.Segment!].push(unmarshalledRecord);
        if (docketEntries[params.Segment!].length >= BATCH_SIZE) {
          console.log(
            `Migrating ${BATCH_SIZE} DocketEntry records for scan segment ${params.Segment!}`,
          );
          await upsertDocketEntries(docketEntries[params.Segment!]);
          docketEntries[params.Segment!].length = 0;
          console.log(
