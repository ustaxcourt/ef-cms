...
CURRENT_ORDER_SEARCH_ENABLED_VALUE=$(aws dynamodb get-item --region us-east-1 --table-name ${DEPLOY_TABLE_NAME} --key '{"pk":{"S":"internal-order-search-enabled "},"sk":{"S":"internal-order-search-enabled "}}' | jq -r ".Item.current.S")
aws dynamodb put-item --region us-east-1 --table-name ${DEPLOY_TABLE_NAME} --item '{"pk":{"S":"internal-order-search-enabled "},"sk":{"S":"internal-order-search-enabled "},"current":{"BOOL": '${CURRENT_ORDER_SEARCH_ENABLED_VALUE}'}}'
