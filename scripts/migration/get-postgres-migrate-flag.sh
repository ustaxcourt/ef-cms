...
MIGRATE_FLAG=$(aws dynamodb get-item --region us-east-1 --table-name "efcms-deploy-${ENV}" --key '{"pk":{"S":"postgres-migrate"},"sk":{"S":"postgres-migrate"}}' | jq -r ".Item.current.BOOL")
