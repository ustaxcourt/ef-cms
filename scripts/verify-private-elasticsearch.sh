...
endpoint=$(aws es describe-elasticsearch-domain --domain "efcms-search-$ENV-alpha" --region us-east-1 | jq -r ".DomainStatus.Endpoint" )
...
endpoint=$(aws es describe-elasticsearch-domain --domain "efcms-search-$ENV-beta" --region us-east-1 | jq -r ".DomainStatus.Endpoint" )
