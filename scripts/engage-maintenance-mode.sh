...
# - Sets MAINTENANCE_MODE=true on all three
# - If any value changed, invokes ./scripts/postgres/set-maintenance-mode.ts true to send notifications
# - If MAINTENANCE_MODE is already true on all three, notifications are skipped
#
# Prerequisites:
...
#     ./scripts/engage-maintenance-mode.sh
#   npm run maintenance:engage
...
if [ "$NEEDS_NOTIFY" = true ]; then
  ...
  echo "MAINTENANCE_MODE already set to true on all lambdas; skipping notifications."
fi
