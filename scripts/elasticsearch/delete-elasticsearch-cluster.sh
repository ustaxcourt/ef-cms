...
if [[ "${SOURCE_ELASTICSEARCH}" == *'alpha'* ]]; then
  DOMAIN_TO_DELETE='alpha'

  SHOULD_ES_ALPHA_EXIST=false
  SHOULD_ES_BETA_EXIST=true
else
  DOMAIN_TO_DELETE='beta'
...
terraform destroy -target 'module.ef-cms_apis.module.elasticsearch_alpha[0].module.logs_alarms'
terraform destroy -target 'module.ef-cms_apis.module.elasticsearch_alpha[0].aws_cloudwatch_log_resource_policy.allow_elasticsearch_to_write_logs'
terraform destroy -target 'module.ef-cms_apis.module.elasticsearch_alpha[0].module.logs_alarms.data.aws_caller_identity.default'
terraform destroy -target 'module.ef-cms_apis.module.elasticsearch_alpha[0].aws_cloudwatch_log_resource_policy.allow_elasticsearch_to_write_logs'