...
LOCAL_IMAGE_EXISTS=$(podman images -q "$LOCAL_IMAGE_NAME" 2> /dev/null)
BUILD_IMAGE=1
...
[[ "$BUILD_IMAGE" -eq 1 ]] && podman build --platform=linux/amd64 --no-cache -t "$LOCAL_IMAGE_NAME" -f Dockerfile .
