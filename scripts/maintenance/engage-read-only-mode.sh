...
# Engages Read-Only Mode for the currently active color in the specified ENV.
#
# What this does:
...
# - Sets READ_ONLY_MODE=true on all three
# - If any value changed, invokes ./scripts/maintenance/set-read-only-mode.ts true to send notifications
# - If READ_ONLY_MODE is already true on all three, notifications are skipped
#
# Prerequisites:
...
if [ "$NEEDS_NOTIFY" = true ]; then
  ./scripts/maintenance/set-read-only-mode.ts true || echo "Warning: Failed to execute set-read-only-mode. If the lambda hasn't been deployed yet, this will fail safely."
else
  echo "READ_ONLY_MODE already set to true on all lambdas; skipping notifications."
fi
