...
# - Sets READ_ONLY_MODE=true on ALL lambdas matching *_${ENV}_${CURRENT_COLOR}
# - If any value changed, invokes ./scripts/maintenance/set-read-only-mode.ts true to send notifications
# - If READ_ONLY_MODE is already true on all three API lambdas, notifications are skipped
#
# Prerequisites:
...
if [ "$NEEDS_NOTIFY" = true ]; then
  ./scripts/maintenance/set-read-only-mode.ts true
else
